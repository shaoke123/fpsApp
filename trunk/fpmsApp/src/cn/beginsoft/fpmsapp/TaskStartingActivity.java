...
public class TaskStartingActivity extends BaseActivity {
    ...
    private void menuInit(){
		...
		//字体颜色
  		...
  		LinearLayout ll_task_starting=(LinearLayout)findViewById(R.id.ll_task_starting);
  		ll_task_starting.setBackgroundColor(this.getResources().getColor(R.color.jblue));
  		//下划线
  		TextView tv_task_starting=(TextView)findViewById(R.id.tv_task_starting);
  		tv_task_starting.setTextColor(this.getResources().getColor(R.color.green)); 
    ...
    private void dataFill() {
    	...
    	params.put("proceState", EProceState.STARTING.getNum());
    	//异步访问
    ...
    private void setListView(){
	    ...
	    for(int position=0;position<jsonArray.length();position++){
			try {
	    		...
	    		Toast.makeText(TaskStartingActivity.this,"暂无数据", Toast.LENGTH_SHORT).show(); 
	    	}
    ...
    private class MySimpleAdapter extends BaseAdapter{
    	...
    	private String formatProceState(View convertView,final String proceState,final String id){
    		...
    		Button start = (Button)convertView.findViewById(R.id.start_btn);
    			str=EProceState.STARTING.getValue();
    			//设置行样式
    			...
    			start.setOnClickListener(new OnClickListener() {
	    		    ...
	    		    public void onClick(View arg0) {
	    		    	params.put("id", id);
	    		    	params.put("proceState","3");
	    		    	params.put("timsapp", "proce_stop_time");
	    		    	aSyncHttpClient.post(RequestURL.BASEURL+RequestURL.TASKUPDATE, params,new AsyncHttpResponseHandler()
	    		        {
	    		            ...
	    		            public void onSuccess(String content)
	    		            {
	    		            	 if(Integer.parseInt(content.trim())>0){
	    			    		    	Toast.makeText(context,"恭喜任务完成", Toast.LENGTH_SHORT).show();
	    			    		    	//刷新界面
	    			    				Intent intent = new Intent(context,TaskStartingActivity.class);  
	    			    				intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);  
	    			                    //跳转Activity  
